...
double machin_formula(int n) {
    ...
    double pi = 16 * sum(v1, n) - 4 * sum( v2, n );
    free(v1);
    free(v2);
    return pi;
}
...
void verification_test(char *test_name, int n) {
  ...
  if ( n < 25 ) {
    for ( size_t k=1; k <= n; k++ ) {
      ...
      fprintf(pFile, "k = %zu \t Mach error = %.10e \n", k,  error_m);
      fprintf(pFile, "k = %zu \t Zeta error = %.10e \n", k,  error_z);
      fputs( "**************************\n", pFile );
    }
  }
...
int main(int argc, char const **argv) {
    int n = atoi(argv[1]);
    verification_test("machin", n);
    verification_test("reimann", n);
